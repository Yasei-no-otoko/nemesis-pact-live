...
import {MODEL_FORMAT,modelSchema,decodeModelContract} from './covenant-model.mjs';
...
export async function handle(request,{env=process.env,fetcher=fetch,now=Date.now,...overrides}={}) {
 ...
 try {
  ...
  const response=await fetcher(gateway?'https://ai-gateway.vercel.sh/v1/responses':'https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:'Bearer '+(gateway?env.AI_GATEWAY_API_KEY:env.OPENAI_API_KEY),'Content-Type':'application/json'},signal:abort.signal,body:JSON.stringify({model:(gateway?'openai/':'')+env.OPENAI_MODEL,store:false,max_output_tokens:700,instructions:INSTRUCTIONS,input:[{role:'user',content:JSON.stringify(clean)}],text:{format:{type:'json_schema',name:MODEL_FORMAT,strict:true,schema:modelSchema()}}})});
  ...
  const validated=decodeModelContract(JSON.parse(output)),budget=V.budget(validated);
