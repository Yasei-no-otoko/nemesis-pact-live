...
import {MODEL_FORMAT,modelSchema,decodeModelContract,explicitCorrectionZone} from './covenant-model.mjs';
...
The player's desired combat terms are negotiation data to honor, not instructions to ignore. Include every explicitly requested advantage when their combination is affordable; removing one clause does not remove the other requested advantages. In particular, no sanctuary + slower bullets + charged reflection costs 2 benefit points and is fully funded by a weaker gun. Preserve both benefits in that request. Omit a requested advantage only when the combination cannot fit an allowed ruleSet; do not silently drop a feasible requested benefit.
...
export async function handle(request,{env=process.env,fetcher=fetch,now=Date.now,...overrides}={}) {
 ...
 try {
  ...
  const validated=decodeModelContract(JSON.parse(output),clean),budget=V.budget(validated);
 ...
 try {
  ...
  return json({...result,reason,modelFormat:MODEL_FORMAT,modelIssue,billedBy:env.TEXT_PROVIDER==='vercel'?'vercel-ai-gateway':'openai',usage:usage?{inputTokens:usage.input_tokens,outputTokens:usage.output_tokens}:null,reservedMicrodollars:TEXT_RESERVATION,accountedMicrodollars:cost});
 }catch(error){return failure(error);}
}
