...
Node 22.23.2: 286/286 tests passed (24553.2922 ms), 53 source syntax checks, standalone and hosted build passed. Unchanged core byte identity, existing API/security suites and offline CSP checks pass. The obsolete English-only text assertions were updated for this explicitly requested bilingual UI.
...
[Local browser evidence](validation-japanese/local/result.json) · [Unit summary](validation-japanese/unit-summary.txt). Production verification and deployment identifiers will be appended after deployment.
