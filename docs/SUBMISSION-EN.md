...
GPT-Live-1 handles spoken English or Japanese and delegates terms to GPT-5.6 Luna through Vercel AI Gateway. Both spoken and typed requests use the same bounded contract validator and explicit Sign action. Model speech and prose cannot modify combat directly. In the latest production integration, synthetic speech interrupted actual Live output, changed left to right, generated Luna contracts, and signed both the initial deal and a spoken mid-fight amendment. Two enemy bullets were erased, health and boss damage were preserved, and both Live sessions terminated. This is real API evidence with automated input, distinct from human microphone footage. Earlier human desktop footage shows spoken amendment; an iPhone clip shows interruption but signs a LOCAL RULES fallback. The generation faults were fixed and the player confirmed the corrected OPENAI sequence in Windows Vivaldi. Those earlier human runs used gpt-4.1-mini and are not relabeled as Luna.
...
The implementation preserves the six-sector game and rendering architecture while adding a bounded proposal ledger, durable access controls, visible contract effects, voice lifecycle handling, and before/after previews. Production deployment: `dpl_8xZnyqCVV3Dw7jRoXEAxdHaVf5gP`. The recorded unit suite passes 209 tests; real production API checks cover replay, interruption, corrected Sign, voice amendment and confirmed termination. Five new Live sessions across three diagnostic attempts have durable confirmed stops; failed attempts are retained. A human 60-second English-captioned video is saved locally, but its main sequence lacks interruption. The phone supplement includes interruption with a clearly labeled fallback. A single full-sequence recording and approved public viewing URL are still outstanding. Source publication or judge access also remains pending.
