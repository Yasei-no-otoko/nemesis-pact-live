...
with sync_playwright() as pw:
    ...
    for name,width,height,mobile in [('desktop',1280,800,False),('portrait',390,844,True),('small',320,568,True)]:
        ...
        context.close()
    (OUT/'browser-results.json').write_text(json.dumps({'build':'0.5.0','htmlSha256':hashlib.sha256(HTML.encode()).hexdigest(),'browser':browser.version,'backend':'ANGLE / SwiftShader SOFTWARE','nativeWebGPU':'NOT EXECUTED','results':results},indent=2))
