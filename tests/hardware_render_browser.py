...
import json,time,os
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'docs/validation-current/hardware-render';OUT.mkdir(parents=True,exist_ok=True)
with sync_playwright() as pw:
 b=pw.chromium.launch(executable_path=r'C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe',headless=True)
 cases=[]
 for name,url,viewport,mobile in [('webgl2-desktop','https://nemesis-pact-live.vercel.app/?renderer=webgl2',{'width':1280,'height':800},False),('auto-desktop','https://nemesis-pact-live.vercel.app/',{'width':1280,'height':800},False),('auto-portrait','https://nemesis-pact-live.vercel.app/',{'width':390,'height':844},True)]:
  ...
  p.click('#first-contact');p.click('#cv-open-connection');p.click('#cv-open-connection');p.select_option('#cv-mode','local');p.click('#cv-connection-done');p.click('#cv-connection-done');p.click('#cv-propose');p.click('#cv-sign');p.wait_for_function('()=>JSON.parse(render_game_to_text()).phase==="combat"')
  if not mobile:
 ...
 report={'checkedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'browser':b.version,'physicalHost':'Windows / AMD Radeon RX 6900 XT (CIM driver 32.0.21045.5002)','method':'automated ordinary LOCAL RULES controls; actual production rendering; 10-second rAF samples; no real phones, no synthetic time, no model calls; rAF is frame cadence, not GPU timestamp query','cases':cases};(OUT/'result.json').write_text(json.dumps(report,indent=2),encoding='utf-8');print(json.dumps({'browser':report['browser'],'samples':[{'case':c['name'],'backend':c['sample']['renderer']['backend'],'fps':c['sample']['renderFps'],'adapter':c['rendererInfo']['glAdapter'],'errors':c['errors'],'apiRequests':len(c['apiRequests'])} for c in cases]},indent=2));b.close()
