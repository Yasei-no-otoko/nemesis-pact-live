"""Latest production media refresh: full campaign, tab capture, and one real Luna debrief.
...
if (OUT/'result.json').exists() or VIDEO.exists():raise SystemExit('Refusing to overwrite an existing media-refresh result or recording.')
...
IDS_FILE=ROOT/'.work/media-refresh-20260915-voice-ids.json'
def production_hash():
 try:return hashlib.sha256(urllib.request.urlopen(URL,timeout=20).read()).hexdigest()
 except Exception:return None
report={'checkedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'url':URL,'sourceCommit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'voiceModel':'gpt-live-1','contractModel':'gpt-5.6-luna','adaptiveModel':'gpt-5.6-luna','debriefModel':'gpt-5.6-luna','humanMicrophone':False,'physicalPhone':False,'headless':True,'operation':'Native Edge WebGPU, ordinary requestAnimationFrame, fixed demo autoplay, tab capture at 1920x1080','controlledTime':False,'mutatedGameState':False,'playerSpeech':'Original Microsoft Zira Desktop synthesis fed into a live Web Audio MediaStream; synthetic voice disclosed in metadata','maxReservationsUsd':{'openai':.30,'gateway':.43},'fixtureSha256':{n:hashlib.sha256((FIX/(n+'.wav')).read_bytes()).hexdigest() for n in audio},'productionHtmlSha256Before':production_hash(),'marks':[],'negotiations':[]}
with sync_playwright() as pw:
 ...
 admitted={'voice':0,'pact':0,'analysis':0,'debrief':0};limits={'voice':6,'pact':24,'analysis':18,'debrief':1};calls=[];errors=[];ids=[];failed=[]
 def admission(route):
  r=route.request;body=r.post_data_json or {};kind='voice' if r.url.endswith('/voice/start') else 'analysis' if r.url.endswith('/adaptive') else 'debrief' if r.url.endswith('/debrief') else 'pact' if body.get('action')=='propose' else None
  if kind:
   admitted[kind]+=1
   if admitted[kind]>limits[kind]:route.fulfill(status=429,json={'error':'CAPTURE_ALLOWANCE_REACHED'});return
  route.continue_()
 for path in ['voice/start','campaign','adaptive','debrief']:page.route('**/api/'+path,admission)
 def response(r):
  ...
  try:
   ...
   if r.url.endswith('/debrief'):item['action']='debrief'
   if r.url.endswith('/voice/start') and body.get('sessionId'):
    ids.append(body['sessionId']);IDS_FILE.write_text(json.dumps(ids),encoding='utf-8')
 ...
 def mark(label,s=None):
  s=s or state();x={'label':label,'captureSeconds':page.evaluate('(Date.now()-NemesisDemo.startedAt)/1000'),'screen':s.get('screen'),'stage':s.get('stage'),'wave':s.get('wave'),'combatSeconds':s.get('seconds'),'bosses':s.get('bossKills'),'hp':(s.get('player') or {}).get('hp'),'level':(s.get('adaptive') or {}).get('level')};report['marks'].append(x);(OUT/'progress-marks.json').write_text(json.dumps(report['marks'],ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(x),flush=True);return x
 ...
 def capture_debrief():
  page.click('#debrief');page.wait_for_function('()=>!document.querySelector("#debrief-screen").hidden')
  assert page.locator('#debrief-provider').inner_text().startswith('LOCAL RULES')
  page.select_option('#debrief-mode','server');page.check('#debrief-consent');page.screenshot(path=str(OUT/'flight-debrief-recorded.png'));page.click('#debrief-analyze')
  page.wait_for_function('()=>document.querySelector("#debrief-provider").textContent.startsWith("OPENAI")&&!JSON.parse(render_game_to_text()).debrief.busy',timeout=30000)
  d=state();assert d['debrief']['provider']=='openai' and d['debrief']['model']=='gpt-5.6-luna';assert admitted['debrief']==1;report['debrief']=d;page.screenshot(path=str(OUT/'flight-debrief-luna.png'));mark('debrief-ready',d);page.wait_for_timeout(5000)
 recording=False
 try:
  page.goto(URL+'/?demo=1',wait_until='load');page.wait_for_function('()=>NEMESIS_RENDERER.stats().backend.includes("WEBGPU")');assert not page.evaluate('!!window.__PACT_TEST__');page.select_option('#title-language','en');page.wait_for_timeout(100);assert page.evaluate('PactI18n.locale')=='en'
  ...
  page.select_option('#demo-mode','display');page.click('#demo-record');page.wait_for_function('()=>NemesisDemo.state==="recording"');recording=True
  report['captureSettings']=page.evaluate('()=>{const s=NemesisDemo.display.getVideoTracks()[0].getSettings();return {width:s.width,height:s.height,frameRate:s.frameRate,displaySurface:s.displaySurface};}');assert report['captureSettings']=={'width':1920,'height':1080,'frameRate':60,'displaySurface':'browser'}
  mark('recording-start');page.wait_for_timeout(1500);page.click('#launch');deadline=time.monotonic()+1200;seen=set();lastkey=None;waveStarted={};bossStarted={};adaptiveShots=set()
  while time.monotonic()<deadline:
   ...
   if s.get('screen')=='game':
    waveKey=(s.get('stage'),s.get('wave'));waveStarted.setdefault(waveKey,time.monotonic())
    if s.get('wave')==1 and time.monotonic()-waveStarted[waveKey]>=3 and s['stage'] not in report.setdefault('wave2Shots',[]):page.screenshot(path=str(OUT/f'wave2-{s["stage"]+1}.png'));report['wave2Shots'].append(s['stage']);mark('wave2-screenshot',s)
    if s.get('wave')==2:
     bossStarted.setdefault(s['stage'],time.monotonic())
     if time.monotonic()-bossStarted[s['stage']]>=3 and s['stage'] not in report.setdefault('bossShots',[]):page.screenshot(path=str(OUT/f'boss-{s["stage"]+1}.png'));report['bossShots'].append(s['stage']);mark('boss-screenshot',s)
   if s.get('screen')=='adaptive-screen' and (s.get('adaptive') or {}).get('history') and s['stage'] in [0,2,5]:
    ak=(s['stage'],s.get('wave'),len(s['adaptive']['history']))
    if ak not in adaptiveShots:page.screenshot(path=str(OUT/f'analysis-{s["stage"]}-{s["wave"]}.png'));adaptiveShots.add(ak)
  ...
  try:
   if recording and page.evaluate('NemesisDemo.state==="recording"'):
    ...
    if report.get('captureEndSettings')!=report.get('captureSettings'):raise AssertionError('Capture settings drifted during recording')
  except Exception as e:report['captureError']=str(e)
  report['productionHtmlSha256After']=production_hash();report.update(api=calls,pageErrors=errors,failedRequests=failed,admitted=admitted,browser=browser.version)
