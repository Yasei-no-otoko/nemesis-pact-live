...
ROOT=Path(__file__).resolve().parents[1];URL='https://nemesis-pact-live.vercel.app';OUT=Path(os.environ.get('NEMESIS_CAMPAIGN_VOICE_OUT',ROOT/'docs/validation-campaign/live-voice'));OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as pw:
 ...
 calls=[];errors=[];ids=[];failed=[];admitted={'voice':0,'text':0}
 def admission(route):
 ...
 def response(r):
  ...
  try:
   ...
   if r.url.endswith('/api/voice/start') and b.get('sessionId'):
    ids.append(b['sessionId']);Path(os.environ.get('NEMESIS_CAMPAIGN_VOICE_IDS',ROOT/'.work/campaign-voice-ids.json')).write_text(json.dumps(ids),encoding='utf-8')
  except Exception:pass
  calls.append(item)
 page.on('requestfailed',lambda r:failed.append({'path':r.url.split(URL)[-1],'failure':r.failure}));page.on('response',response);page.on('pageerror',lambda e:errors.append(str(e)))
 try:
  ...
  report.update(api=calls,pageErrors=errors,failedRequests=failed,browser=browser.version,admitted=admitted)
