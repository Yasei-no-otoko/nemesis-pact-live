...
import json
R=Path(__file__).resolve().parents[1]; out=evidence_dir('browser/covenant-smoke')
with sync_playwright() as pw:
 ...
 for name,w,h,mobile in [('desktop',1280,800,False),('portrait',390,844,True),('small',320,568,True)]:
  ...
  p.set_content((R/'dist/NEMESIS-PACT.html').read_text(encoding='utf-8'));p.evaluate('async()=>{await NEMESIS_RENDERER.initialized;__PACT_TEST__.render(3)}')
