...
try{
 ...
 res=await fetch(base+'/api/intelligence',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)});assert.equal(res.status,200);const legacy=await res.json();assert.equal(legacy.provider,'mock');assert.equal(legacy.fallback,false);assert.ok(request.allowed.includes(legacy.decision.contractId));checks.push('Legacy endpoint returns only its deterministic mock in AI_MODE=mock');
