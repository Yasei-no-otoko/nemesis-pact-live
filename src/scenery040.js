...
class Scene extends M.Scene{
 ...
 covenantFloor(w,h,seed){
  this.theme=0;const r=rng(seed+':covenant-court'),ink=[.010,.020,.032],alloy=[.050,.092,.115],brass=[.23,.18,.10],mint=[.10,.48,.42],cx=w/2,cy=h*.43,unit=Math.min(w,h);
  this.box(cx,h/2,-119,w+180,h+180,6,0,ink,.94,.05,0,LAYER.GROUND);
  // Recessed strips read as architecture, not collidable platforms.
  for(let j=0;j<8;j++){const x=w*(.14+j*.102);this.box(x,h/2,-108,w*.087,h*.91,5,0,[.014,.029,.043],.83,.2,0,LAYER.GROUND);}
  for(let i=0;i<60;i++){const a=i*TAU/60;for(const rr of [unit*.29,unit*.42])this.box(cx+Math.cos(a)*rr,cy+Math.sin(a)*rr,-91,i%5===0?12:5,1.6,2,a, i%5===0?brass:alloy,.57,.5,i%5===0?.1:0,LAYER.TRIM);}
  for(let i=0;i<8;i++){const a=i*TAU/8,rr=unit*.49,x=cx+Math.cos(a)*rr,y=cy+Math.sin(a)*rr;
   this.add('hex',x,y,-78,26,26,16,a,alloy,.48,.65,0,LAYER.LOW);
   this.box(x,y,-64,26,3,4,a,brass,.38,.68,.08,LAYER.LOW);
   this.add('orb',x,y,-59,3.1,3.1,4,0,mint,.4,.1,.7,LAYER.TRIM);
  }
  for(const side of [-1,1])for(let j=0;j<4;j++){const x=cx+side*w*.454,y=h*(.17+j*.205);
   this.add('hex',x,y,-48,24,42,44,side*.15,alloy,.52,.55,0,LAYER.HIGH);
 ...
 build(frame){this.living=frame.world?.mode==='first-contact';this.airframe=frame.world?.airframe||frame.airframe||'vanguard';return super.build(frame);}
}
