...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./core.js'),require('./expansion.js'));else root.PactCovenant=factory(root.PactCore,root.PactExpansion);})(typeof globalThis!=='undefined'?globalThis:this,function(C,X){'use strict';
 ...
 function schema(){const str=n=>({type:'string',minLength:1,maxLength:n});return {type:'object',additionalProperties:false,required:SPEC_KEYS,properties:{version:{type:'integer',enum:[1]},title:str(48),zone:{type:'string',enum:ZONES},speed:{type:'string',enum:SPEEDS},reflection:{type:'string',enum:REFLECTS},price:{type:'string',enum:PRICES},line:str(240),rationale:str(360)}};}
 ...
 function localProposal(request){const r=cleanRequest(request),p=r.prompt.toLowerCase();let zone=/sanctuary|safe|shield|\u5b89\u5168|\u7d50\u754c/u.test(p)?'center':'none';
 ...
 function revisionImpacts(w){
  const receipts=Array.isArray(w?.receipts)?w.receipts:[],final=telemetry(w||{}),breach=w?.broken&&w?.breachSnapshot?w.breachSnapshot:null;
  return receipts.map((receipt,index)=>{
   const before=receipt.snapshot||{},after=receipts[index+1]?.snapshot||(index===receipts.length-1&&breach)||final;
   const delta=key=>Math.max(0,(Number(after[key])||0)-(Number(before[key])||0));
   return {revision:receipt.revision,shielded:delta('shielded'),reflectedDamage:delta('reflectedDamage')};
  });
 }
 class Client{
  constructor(){this.sequence=0;this.abort=null;}
  cancel(){this.sequence++;this.abort?.abort();this.abort=null;}
  async propose(raw,{mode='local',consent=false,session,runId,intentVersion}={}){const r=cleanRequest(raw);this.cancel();const sequence=this.sequence,start=performance.now(),fallback=reason=>({spec:localProposal(r),provider:'local-rules',fallback:mode==='server',reason,model:null,latencyMs:Math.round(performance.now()-start)});
   ...
   const controller=new AbortController();this.abort=controller;const timer=setTimeout(()=>controller.abort(),18000);
   try{if(!session)throw Error('Session unavailable');await session.ensure();if(sequence!==this.sequence)throw Error('Superseded');const requestId=crypto.randomUUID();const res=await session.request('/api/covenant',{request:r,runId,intentVersion,requestId},{signal:controller.signal});if(!res.ok)throw Error(res.status===409?'Superseded':res.status===429?'Budget or concurrency limit reached':'Server unavailable ('+res.status+')');const o=await res.json();if(!['local-rules','openai'].includes(o.provider))throw Error('Unknown provider');const spec=validateSpec(o.spec);if(sequence!==this.sequence)throw Error('Superseded');if(o.proposalId&&(o.intentVersion!==intentVersion||o.requestId!==requestId))throw Error('Superseded');return {spec,provider:o.provider,proposalId:o.proposalId||null,digest:o.digest||null,intentVersion,requestId,fallback:o.provider!=='openai',reason:o.provider==='openai'?'Validated OpenAI response.':'LOCAL RULES: upstream unavailable or invalid.',model:o.provider==='openai'?text(o.model,100):null,latencyMs:Math.round(performance.now()-start)};
   }catch(e){if(sequence!==this.sequence||e.message==='Superseded')throw Error('Superseded');return fallback('LOCAL RULES: '+(e.name==='AbortError'?'Timed out; local counteroffer.':String(e.message).slice(0,120)));}finally{clearTimeout(timer);if(this.abort===controller)this.abort=null;}
  }
 }
 class Run extends X.Run{
  constructor(seed='FIRST-CONTACT',difficulty='standard',viewport={},history){super(seed,difficulty,false,viewport,{mode:'classic'});this.mode='first-contact';this.totalStages=1;this.phase='covenant';this.wave=2;this.credits=0;this.upgrades={rail:1,parry:1};this.p.hp=this.p.maxHp=difficulty==='veteran'?7:10;this.p.energy=50;this.spec=null;this.revision=0;this.amendments=1;this.autoParley=true;this.receipts=[];this.breachSnapshot=null;this.memory=memory(history);this.covenantStats={shielded:0,reflectedDamage:0,gunDamage:0,amendments:0};this.voiceEvidence={model:'gpt-live-1',connectionStarted:0,sessionClosed:0,delegationRequests:0,proposalsReturned:0,openaiProposals:0,signedContracts:0,lastState:'idle'};this.savedMemory=false;this.finishReason=null;}
  ...
  breach(){const ok=super.breach();if(ok){this.breachSnapshot=telemetry(this);this.autoParley=false;this.amendments=0;}return ok;}
  ...
  report(){const voice={...this.voiceEvidence};return {...super.report(),version:'1.3.0',mode:this.mode,contractLanguage:'living-covenant-v1',assists:'automatic aim/fire in First Contact UI',rules:this.spec?{...this.spec}:null,covenantStats:{...this.covenantStats},receipts:this.receipts.map(r=>({...r,spec:{...r.spec},snapshot:{...r.snapshot}})),breachSnapshot:this.breachSnapshot?{...this.breachSnapshot}:null,memoryBefore:{...this.memory},memoryAfter:this.memoryAfter(),liveVoice:{...voice,status:voice.signedContracts>0?'contract-signed':voice.connectionStarted>0?'connected':'not-used'}};}
 }
 return {ZONES,SPEEDS,REFLECTS,PRICES,validateSpec,cleanRequest,schema,compile,describe,budget,localProposal,memory,telemetry,revisionImpacts,Client,Run};
});
