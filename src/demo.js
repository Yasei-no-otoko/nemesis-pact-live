 ...
 class DemoRecorder{
  ...
  async start(){
   ...
   if(!this.navigator?.mediaDevices?.getDisplayMedia||!this.MediaRecorder){this._state('error','capture-unavailable');return false;}
    ...
    try{
     ...
     const display=await this.navigator.mediaDevices.getDisplayMedia({video:{frameRate:60,width:{max:1920},height:{max:1080}},audio:false,preferCurrentTab:true,selfBrowserSurface:'include'});
  ...
  _cleanup(session,reason='cancelled'){
   ...
   const tracks=new Set([...(session.display?.getTracks?.()||[]),...(session.recordStream?.getTracks?.()||[]),...(session.destination?.stream?.getTracks?.()||[])]);
    tracks.forEach(t=>t.stop?.());try{this.gameAudioRelease?.();}catch{}this.gameAudioRelease=null;this.gameAudioSource=null;try{this.voiceOutputGain?.disconnect?.();}catch{}this.voiceOutputGain=null;this.voiceOutputElement?.removeEventListener?.('volumechange',this._voiceVolumeChanged);this.voiceOutputBound=false;try{session.audioContext?.close?.()?.catch?.(()=>{});}catch{}
  ...
  const singleton=new DemoRecorder();singleton.DemoRecorder=DemoRecorder;for(const k of ['attachVoice','registerGameAudio','attach','start','stop'])singleton[k]=singleton[k].bind(singleton);return singleton;
});
