...
(function (root, factory) {
  ...
  class World {
    ...
    protectBullet(b){return this.mods.sanctuary&&segmentHit(b.px,b.py,b.x,b.y,this.width/2,this.height/2,76);}
    ...
    step(dt,input={}){
      ...
      for(const b of this.bullets){
        ...
        if(b.hostile){
          if(this.protectBullet(b)){b.life=0;continue;}
          const d=dist(b,p);
    ...
    report(){return {version:'0.5.0',layout:this.layout,arena:{width:this.width,height:this.height},seed:this.seed,difficulty:this.difficulty,outcome:this.phase,score:this.score,seconds:+this.time.toFixed(2),kills:this.kills,bosses:this.bossKills,parries:this.parries,grazes:this.grazes,damageTaken:this.damageTaken,breaches:this.breaches,contracts:this.contracts.map(c=>({...c})),upgrades:{...this.upgrades},timeline:this.log.slice(-256)};}
  }
