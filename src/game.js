...
(function(){'use strict';
  ...
  function updateCovenantHUD(){const active=world?.mode==='first-contact';$('covenant-live').hidden=!active||screen!=='game';$('pause-parley').hidden=!active;$('combat-guide').hidden=!active||screen!=='game'||mobile;if(!active){$('touch-context-hint').textContent='Release to stop';return;}
    ...
    const cue=window.PactCovenantReview.combatCue(world);
    $('combat-guide').dataset.cue=cue.kind;$('combat-guide-action').textContent=T(cue.action);$('combat-guide-detail').textContent=T(cue.detail);$('touch-context-hint').textContent=T(cue.touchHint);$('touch-context-hint').dataset.cue=cue.kind;
  }
