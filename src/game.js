...
(function(){'use strict';
  ...
  function renderCovenantResult(won){
    ...
    $('result-pacts').innerHTML=world.receipts.map((r,i)=>`<div class="result-pact ${world.broken&&i===world.receipts.length-1?'broken':''}"><span>REV ${r.revision} / ${escapeHTML(r.spec.title)}</span><small>${i<world.receipts.length-1?'REVISED':world.broken?'BROKEN':won?'HONORED':'SIGNED'}</small></div>`).join('');
    $('result-build').textContent='Sanctuary interceptions and reflected hit damage, across all revisions. '+(world.receipts.some(r=>r.provider==='openai')?'Includes validated OpenAI contracts.':'Local-rule play; no AI inference.');
  }
