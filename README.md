...
The optional `?demo=1` recorder starts only on explicit click and browser screen-share selection. It records the chosen game tab and active game microphone locally until **Stop & save**, shows elapsed time, and downloads WebM. There is no fixed recording duration; capture through boss defeat, then edit to at most 60 seconds in DaVinci Resolve. See [the recording and Resolve guide](docs/FINAL-RECORDING.md). Nothing is uploaded automatically. A recorder timer defect in the first human attempt prevented saving; the fix passed a 60-second real MediaRecorder test with explicitly synthetic capture. The 60-second human English-captioned MP4 is now saved and playback-verified locally, but its public viewing URL remains pending approval/hosting and the clip itself does not show interruption during the Notary's reply. The 50.47-second iPhone supplement shows interruption, but its corrected Sign used LOCAL RULES, so it is not a replacement for a single 60-second all-mechanics demo. The repository remains private pending a source-publication or judge-access decision.

Newer human Luna footage shows corrected right Sign and combat, but exposed a mid-fight clause-withdrawal bug. That bug, missing-delegation recovery and omitted affordable benefits are fixed. The current release passed the exact Japanese amendment and a full real Live/Luna interruption/sign/combat/amendment regression with synthetic input. A successful full-sequence human recording remains outstanding; see `docs/validation-current/voice-withdrawal/` for passing and failed evidence.
